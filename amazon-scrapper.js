...
class AmazonScraper {
  ...
  get getProxy() {
    const selectProxy =
      Array.isArray(this.proxy) && this.proxy.length ? this.proxy[Math.floor(Math.random() * this.proxy.length)] : ''
  ...
  async startScraper() {
    ...
    return {
      ...(this.scrapeType === 'products'
        ? { totalProducts: this.totalProducts, category: this.productSearchCategory }
        : {}),
  ...
  grabAsinDetails(body) {
    ...
    output.badges.amazon_prime =
      $(`span[id="priceBadging_feature_div"]`) || $(`span[id="priceBadging_feature_div"]`)[0] ? true : false
      ...
      try {
        ...
        if (ratingSearch) {
          ...
          scrapingResult[key].score = parseFloat(
            scrapingResult[key].reviews.rating * scrapingResult[key].reviews.total_reviews
          ).toFixed(2)
        }
      ...
      moreBuyingOptions.children.forEach((item, index) => {
        try {
          if (item.attribs.class && item.attribs.class === 'a-box mbc-offer-row pa_mbc_on_amazon_offer') {
            position += 1
            const price = $(
              `#mbc > div:nth-child(${index + 1}) > div > span.a-declarative > div > div:nth-child(1)`
            ).text()
